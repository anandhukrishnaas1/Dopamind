...
import 'package:dopamind/services/ai_service.dart';
...
class _AnalyticsScreenState extends State<AnalyticsScreen>
  ...
  String _aiInsight = 'Click "Generate Insights" to get AI coaching based on your usage.';
  bool _isLoadingAI = false;

  Future<void> _fetchAIInsight(AppStore store) async {
    setState(() => _isLoadingAI = true);
    
    // Refresh real usage data from system first
    await store.refreshUsageData();

    final insight = await AIService.getAIAnalytics({
      'dopamineScore': store.currentDopamineScore,
      'usage': store.appUsage.map((a) => {
        'app': a.appName,
        'minutes': a.usageMinutes,
        'impact': a.dopamineImpact,
      }).toList(),
    });
    setState(() {
      _aiInsight = insight;
  ...
  Widget build(BuildContext context) {
    ...
    final avgScore = store.weeklyStats.isEmpty ? 0 : store.weeklyStats.fold<int>(0, (a, s) => a + s.dopamineScore) ~/
        store.weeklyStats.length;
    final avgFocus = store.weeklyStats.isEmpty ? 0 : store.weeklyStats.fold<int>(0, (a, s) => a + s.focusMinutes) ~/
        store.weeklyStats.length;
    final totalScroll = store.weeklyStats.isEmpty ? 0 : store.weeklyStats.fold<int>(0, (a, s) => a + s.scrollSessions);
        ...
        child: Column(
          ...
          children: [
                    ...
                    children: [
                      Row(
                        ...
                        children: [
                          Row(
                            children: [
                              const Icon(Icons.auto_awesome, size: 18, color: AppColors.chart1),
                              const SizedBox(width: 8),
                              Text(
                                'AI Insights',
                          ...
                          if (!_isLoadingAI)
                            TextButton(
                              onPressed: () => _fetchAIInsight(store),
            ...
            Padding(
              ...
              child: Row(
                children: [
                  _buildStatCard('$avgScore', 'Avg Score', AppColors.dopamineLow, theme, isDark),
                  const SizedBox(width: 12),
                  _buildStatCard('$avgFocus', 'Focus Min/Day', null, theme, isDark),
                  const SizedBox(width: 12),
                  _buildStatCard('$totalScroll', 'Scroll Sessions', AppColors.dopamineHigh, theme, isDark),
                ],
              ),
            ),
            ...
            Padding(
              ...
              child: SizedBox(
                ...
                child: TabBarView(
                  controller: _tabController,
                  children: [
                    _buildScoreChart(store, theme, isDark, mutedFg, borderCol),
                    _buildFocusChart(store, theme, isDark, mutedFg, borderCol),
                    _buildScreenTimeChart(store, theme, isDark, mutedFg, borderCol),
                  ],
                ),
              ),
            ),
  ...
  Widget _buildStatCard(
      String value, String label, Color? valueColor, ThemeData theme, bool isDark) {
    return Expanded(
      child: Container(
        padding: const EdgeInsets.all(16),
        decoration: BoxDecoration(
  ...
  Widget _buildScoreChart(
      AppStore store, ThemeData theme, bool isDark, Color mutedFg, Color borderCol) {
    return _buildCard(
      theme: theme,
      child: Padding(
        padding: const EdgeInsets.all(16),
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            Expanded(
              child: store.weeklyStats.isEmpty 
                ? Center(child: Text('No data available yet', style: TextStyle(color: mutedFg)))
                : LineChart(
                    LineChartData(
                      gridData: FlGridData(
                        show: true,
                      ...
                      lineBarsData: [
                        LineChartBarData(
                          ...
                          belowBarData: BarAreaData(
                            show: true,
                            color: AppColors.chart2.withOpacity(0.15),
                          ),
                        ),
                      ],
                    ),
            ...
            Row(
              children: [
                const Icon(Icons.trending_up, size: 16, color: AppColors.dopamineLow),
                const SizedBox(width: 8),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                const Icon(Icons.trending_down, size: 16, color: AppColors.dopamineLow),
                const SizedBox(width: 8),
                Text('Real-time data synced', style: TextStyle(color: mutedFg, fontSize: 14)),
              ],
            ),
          ],
        ),
